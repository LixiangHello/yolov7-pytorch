...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            grid_y = torch.linspace(0, input_height - 1, input_height).repeat(input_width,
                                                                              1).t().repeat(
            ...
            anchor_w = anchor_w.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(
                w.shape)
            anchor_h = anchor_h.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(
                h.shape)
            ...
            #   将输出结果归一化成小数的形式
            # ----------------------------------------------------------#
            _scale = torch.Tensor([input_width, input_height, input_width, input_height]).type(
                FloatTensor)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape,
        ...
        for i, image_pred in enumerate(prediction):
            ...
            if output[i] is not None:
                ...
                box_xy, box_wh = (output[i][:, 0:2] + output[i][:, 2:4]) / 2, output[i][:, 2:4] - \
                                 output[i][:, 0:2]
...
class DecodeBoxNP():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            #   batch_size, 3, 80, 80, 85
            # -----------------------------------------------#
            prediction = np.transpose(np.reshape(input, (
            ...
            grid_x = np.repeat(np.expand_dims(
                np.repeat(np.expand_dims(np.linspace(0, input_width - 1, input_width), 0),
                          input_height, axis=0), 0), batch_size * len(self.anchors_mask[i]), axis=0)
            grid_x = np.reshape(grid_x, np.shape(x))
            grid_y = np.repeat(np.expand_dims(
                np.repeat(np.expand_dims(np.linspace(0, input_height - 1, input_height), 0),
                          input_width, axis=0).T, 0), batch_size * len(self.anchors_mask[i]),
                axis=0)
            ...
            #   batch_size,3,20,20
            # ----------------------------------------------------------#
            anchor_w = np.repeat(np.expand_dims(
                np.repeat(np.expand_dims(np.array(scaled_anchors)[:, 0], 0), batch_size, axis=0),
                -1), input_height * input_width, axis=-1)
            anchor_h = np.repeat(np.expand_dims(
                np.repeat(np.expand_dims(np.array(scaled_anchors)[:, 1], 0), batch_size, axis=0),
                -1), input_height * input_width, axis=-1)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape,
        ...
        for i, image_pred in enumerate(prediction):
            ...
            for c in unique_labels:
                ...
                # Add max detections to outputs
                output[i] = max_detections if output[i] is None else np.concatenate(
                    (output[i], max_detections))
