...
def draw_plot_func(dictionary, n_classes, window_title, plot_title, x_label, output_path, to_show,
    ...
    if true_p_bar != "":
        ...
        plt.barh(range(n_classes), fp_sorted, align='center', color='crimson',
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            for idx, detection in enumerate(dr_data):
                ...
                if show_animation:
                    ...
                    if len(ground_truth_img) == 0:
                        ...
                        bottom_border = 60
                        BLACK = [0, 0, 0]
                        img = cv2.copyMakeBorder(img, 0, bottom_border, 0, 0, cv2.BORDER_CONSTANT,
                                                 value=BLACK)
                ...
                if show_animation:
                    ...
                    img, line_width = draw_text_in_image(img, text, (margin + line_width, v_pos),
                    ...
                    if ovmax != -1:
                        ...
                        if status == "INSUFFICIENT OVERLAP":
                            ...
                            text = "IoU: {0:.2f}% ".format(ovmax * 100) + ">= {0:.2f}% ".format(
                                min_overlap * 100)
                            color = green
                        img, _ = draw_text_in_image(img, text, (margin + line_width, v_pos), color,
                                                    line_width)
                    # 2nd line
                    ...
                    if ovmax > 0:
                        ...
                        cv2.rectangle(img_cumulative, (bbgt[0], bbgt[1]), (bbgt[2], bbgt[3]),
                                      light_blue, 2)
                        cv2.putText(img_cumulative, class_name, (bbgt[0], bbgt[1] - 5), font, 0.6,
            ...
            F1 = np.array(rec) * np.array(prec) * 2 / np.where(
                (np.array(prec) + np.array(rec)) == 0, 1, (np.array(prec) + np.array(rec)))
            ...
            if len(prec) > 0:
                print(text + "\t||\tscore_threhold=" + str(
                    score_threhold) + " : " + "F1=" + "{0:.2f}".format(F1[score_threhold_idx]) \
                      + " ; Recall=" + "{0:.2f}%".format(
            ...
            if draw_plot:
                ...
                plt.fill_between(area_under_curve_x, 0, area_under_curve_y, alpha=0.2,
                                 edgecolor='r')
    ...
    if draw_plot:
        ...
        plot_title += "(" + str(len(ground_truth_files_list)) + " files and " + str(
            n_classes) + " classes)"
